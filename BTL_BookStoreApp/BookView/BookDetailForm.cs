    ...
    public partial class BookDetailForm : Form
    {
        ...
        private void BookDetailForm_Load(object sender, EventArgs e)
        {
            ...
            if (SelectedBook != null)
            {
                ...
                txtBookId.Enabled = false; //ko cho edit cột BookId trong cột edit
            ...
            if (cboAuthor.SelectedItem is SpecialItem selectedItem)
            {
                if (selectedItem.Id == "-1")
                {
                    using (var form = new AuthorDetailForm())
                    {
                        ...
                        {
                            new ComboBoxHelper().LoadToComboBox<Author>(cboAuthor, new CabinetService<Author>().GetAll(), a => a.AuthorId, a => a.AuthorName ?? "N");
        ...
        private void cboPublisher_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboPublisher.SelectedItem is SpecialItem selectedItem)
            {
                if (selectedItem.Id == "-1")
                {
                    using (var form = new PublisherDetailForm())
                    {
                        if (form.ShowDialog() == DialogResult.OK)
                        {
                            new ComboBoxHelper().LoadToComboBox<Publisher>(cboAuthor, new CabinetService<Publisher>().GetAll(), a => a.PublisherId, a => a.PublisherName ?? "N");
            ...
            if (cboBookCategory.SelectedItem is SpecialItem selectedItem)
            {
                if (selectedItem.Id == "-1")
                {
                    using (var form = new BookCategoryDetailForm())
                    {
                        if (form.ShowDialog() == DialogResult.OK)
                        {
                            new ComboBoxHelper().LoadToComboBox<BookCategory>(cboBookCategory, new CabinetService<BookCategory>().GetAll(), a => a.BookCategoryId.ToString(), a => a.BookGenreType);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            Repositories.Entities.Book book = new Repositories.Entities.Book()
            {
                ...
                SelltPrince = int.Parse(txtSellPrince.Text),
                ImportPrince = int.Parse(txtImportPrince.Text),
